...
class GraderOrchestrationAgent(DocumentOrchestrationAgent):
    ...
    async def orchestrate(self, document_id: str, commands: ICommandHandlingPort, query: IQueryPort, priority: OrchestrationPriority=None):
        ...
        with await opentelemetry.getSpan("orchestrate_grader") as span: 
            ...
            # Check if orchestration window is open -------------------------------------------------------------------------------------------------------
            current_time = now_utc().time()
            start_time = dt_time.fromisoformat(ORCHESTRATION_GRADER_SCHEDULE_WINDOW_START)
            end_time = dt_time.fromisoformat(ORCHESTRATION_GRADER_SCHEDULE_WINDOW_END)

            # Refactor if the start time is moved back to the previous day.  Currently starttime is midnight
            if ORCHESTRATION_GRADER_SCHEDULE_WINDOW_ENABLED and (current_time < start_time or current_time > end_time):
                LOGGER.info("Orchestration window for %s is closed.  Current time is %s.  Window is open from %s to %s", DocumentOperationType.MEDICATION_GRADER, current_time, start_time, end_time, extra=extra)
                raise WindowClosedException(f"Orchestration window for {DocumentOperationType.MEDICATION_GRADER} is closed.  Current time is {current_time}.  Window is open from {start_time} to {end_time}")
            else:
                LOGGER.debug("Orchestration window for %s is open.  Current time is %s.  Window is open from %s to %s", DocumentOperationType.MEDICATION_GRADER, current_time, start_time, end_time, extra=extra)
            # End orchestration window check -------------------------------------------------------------------------------------------------------
