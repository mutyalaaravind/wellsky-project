...
class DocumentAIAdapter:
    def __init__(
        ...
        project_id: str,
        location: str,
        hcc_processor_id: str,
        hcc_processor_version: str,
        summarizer_processor_id: str,
        summarizer_processor_version: str,
        document_processor_id:str,
    ...
    async def process_document(
        ...
        return await self._process_document_with_document_parser(storage_uri, mime_type, metadata)
    ...
    async def _process_document_with_layout_parser(
        ...
        solution_code = metadata.get("solution_code", "unknown")
        domain_id = metadata.get("domain_id", DEFAULT_DOMAIN_ID)
        app_id = metadata.get("app_id", "none")
        tenant_id = metadata.get("tenant_id", "none")

        # Create billing labels (Document AI requires lowercase, max 63 chars)
        labels = {
            "business-unit": business_unit.lower().replace(" ", "-")[:63],
            "solution-code": solution_code.lower().replace(" ", "-")[:63],
            "domain-id": domain_id.lower().replace(" ", "-")[:63],
            "app-id": app_id.lower().replace(" ", "-")[:63],
            "tenant-id": tenant_id.lower().replace(" ", "-")[:63]
        }

        # Log billing labels for transparency
        LOGGER.debug(f"Using Document AI billing labels (layout parser): {labels}")
        ...
        request = documentai.ProcessRequest(
            ...
            # Only supported for Document OCR processor
            process_options={},  # type: documentai.ProcessOptions
            labels=labels  # Add billing labels
        )

        result = await self.client.process_document(request=request)
        LOGGER.debug(f"Document AI layout parser result received")
    ...
    async def _process_document_with_document_parser(
        ...
        # Log billing labels for transparency
        LOGGER.debug(f"Using Document AI billing labels: {labels}")
