...
from paperglass.settings import GCP_FIRESTORE_DB, JSON_CLEANERS
...
async def do_something():
    test_str = "```json{\"foo\": \"ABCDEFGH\G\\IJKLMNOPQRS\\TUVWXYZ\"}```"

    o = safe_loads(test_str)
    LOGGER.debug("Return object: %s", json.dumps(o, indent=2))
    LOGGER.debug("foo: %s", o["foo"])
