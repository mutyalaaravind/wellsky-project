...
from paperglass.settings import JSON_CLEANERS
...
def safe_loads(jsonStr, index=-1):
    try:
        ...
        if index < 0:
            ...
            #LOGGER.debug("Applying json cleaner: %s", cleaner, extra=extra)
            clean_str = re.sub(cleaner.get("match"), cleaner.get("replace"), jsonStr)
            return json.loads(clean_str)
        else:
            #LOGGER.error("Unable to decode json after cleaning: '%s'  Returning None", jsonStr)
            return None
        ...
        #LOGGER.warning("Error decoding JSON: %s  Attempting to clean string and try again", e, extra=extra)
