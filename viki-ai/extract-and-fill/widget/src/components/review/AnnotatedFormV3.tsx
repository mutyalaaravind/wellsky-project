...
const AnnotatedForm: React.FC<AnnotatedFormProps> = ({ schema, originalData, newData, onPreferenceChange, onEvidenceRequested }) => {
  ...
  console.log('dbg', originalData, newData);
  ...
  const aiValues: any = React.useMemo(() => {
    ...
    const result: any = [];
    const recurse = (originalObj: any, obj: any, path: Array<string | number>) => {
      console.log('dbg', originalObj, obj, path);
      if (Array.isArray(obj)) {
        // Array
        obj.forEach((item, index) => recurse(originalObj[index], item, path.concat(index)));
        ...
        if (obj.hasOwnProperty('value')) {
          ...
          Object.keys(obj).forEach(key => recurse(obj[key], obj[key], path.concat(key)));
        }
      } else {
        result.push([JSON.stringify(path), obj !== null && typeof obj !== 'undefined']);
      }
  ...
  const navigateRelatively = React.useCallback((path: Array<string>, direction: NavDirection) => {
    ...
    if (currentFieldIndex === -1) {
      console.error('navigateRelatively: current path not found in aiValues - ', pathStr);
      return;
    }
  ...
  return (
    <div>
      ...
      */}
      <Section pathStr={'[]'} schema={{"items":[schema]}} originalData={originalData} newData={newData} depth={0} displayMode={displayMode} onPreferenceChange={updatePreference} onEvidenceClicked={requestEvidence} onRelativeNavigation={navigateRelatively} onControllerReady={setFieldController} />
    </div>
  );
};
...
const Section = React.memo(({ schema, originalData, newData, depth, pathStr, displayMode, onControllerReady, onPreferenceChange, onEvidenceClicked, onRelativeNavigation }: SectionProps) => {
  ...
  // const allKeys: Array<string> = schema.items.map((item: any) => item.questionCode);

  const questionLayout = schema?.displayControl?.questionLayout;
  return <Grid gap={2} style={{ display: 'flex', width: '100%', flexDirection: questionLayout === 'horizontal' ? 'row' : 'column' }}>
    {schema.items.map((itemSchema: any) => {
      const isSection = itemSchema?.dataType === 'SECTION';
      // console.log('Rendering', itemSchema, 'isSection =', isSection);
      const key = itemSchema.localQuestionCode || itemSchema.questionCode;
      const originalValue = (originalData !== null && originalData.hasOwnProperty(key)) ? originalData[key] : null;
      const newValue = (newData !== null && newData.hasOwnProperty(key)) ? newData[key] : null;
      const isRepeatable = itemSchema?.questionCardinality?.max === '*';
      ...
      console.log('render', key, isSection, isRepeatable, newValue,newData);

      return (isSection && isRepeatable) ? (
        // Array
        <Grid.Item key={key}>
          ...
          <div style={{ margin: '1rem 0', width: '100%', height: '100%', paddingLeft: '12px', paddingRight: '12px', paddingTop: '16px', paddingBottom: '16px', background: '#D7F1FE', justifyContent: 'space-between', alignItems: 'center', display: 'inline-flex' }} >
            <div style={{ color: 'rgba(0, 0, 0, 0.38)', fontSize: '14px', fontFamily: 'Roboto', fontWeight: '600', lineHeight: '30px', wordWrap: 'break-word' }}>{itemSchema.question} ({itemSchema.localQuestionCode})</div>
          </div>
          {/* </Heading> */}
          <div style={{ borderLeft: `0.25rem solid ${headingColor}`, padding: '0 0 0 1rem' }}>
            {(newValue !== null && newValue.length) ? newValue.map((item: any, index: number) => {
              const originalItemValue = originalValue !== null && originalValue.length > index ? originalValue[index] : null;
              const newItemValue = newValue !== null && newValue.length > index ? newValue[index] : null;
              return (
                <div key={JSON.stringify(originalItemValue) + JSON.stringify(newItemValue)}>
                  <div style={{ margin: '1rem 0', width: '100%', height: '100%', paddingLeft: '12px', paddingRight: '12px', background: '#D7F1FE', justifyContent: 'space-between', alignItems: 'center', display: 'inline-flex' }} >
                    <div style={{ color: 'rgba(0, 0, 0, 0.38)', fontSize: '14px', fontFamily: 'Roboto', fontWeight: '600', lineHeight: '30px', wordWrap: 'break-word' }}>Item {index + 1}</div>
                  </div>
                  <div style={{ borderLeft: `0.25rem solid ${headingColor}`, padding: '1rem 0 0 1rem' }}>
                    <Section pathStr={JSON.stringify(path.concat(key, index))} schema={itemSchema} originalData={originalItemValue} newData={newItemValue} depth={depth + 1} displayMode={displayMode} onPreferenceChange={onPreferenceChange} onEvidenceClicked={onEvidenceClicked} onRelativeNavigation={onRelativeNavigation} onControllerReady={onControllerReady} />
                  </div>
                </div>
              );
      ...
      <Grid.Item key={key}>
          ...
          <div style={{ borderLeft: `0.25rem solid ${headingColor}`, padding: '1rem 0 0 1rem' }}>
            ...
            <Section pathStr={JSON.stringify(path.concat(key))} schema={itemSchema} originalData={originalValue} newData={newValue} depth={depth + 1} displayMode={displayMode} onPreferenceChange={onPreferenceChange} onEvidenceClicked={onEvidenceClicked} onRelativeNavigation={onRelativeNavigation} onControllerReady={onControllerReady} />
          </div>
        </Grid.Item>
      ) : (
        // Field
        displayMode === 'twoCol' ? (
          <>
            ...
            <Grid.Item alignSelf="center">
              <Field pathStr={JSON.stringify(path.concat(key))} schema={itemSchema} name={key} originalData={originalValue} newDataObj={newValue} onPreferenceChange={onPreferenceChange} onEvidenceClicked={onEvidenceClicked} onRelativeNavigation={onRelativeNavigation} onControllerReady={onControllerReady} />
            </Grid.Item>
          </>
        ) : (
          <Grid.Item>
...
const Field = React.memo(({ schema, name, pathStr, originalData, newDataObj, onControllerReady, onPreferenceChange, onEvidenceClicked, onRelativeNavigation }: FieldProps) => {
  const path = JSON.parse(pathStr);
  console.log("newDataObj",newDataObj, name);
  const newData = newDataObj?.hasOwnProperty('value') ? newDataObj.value : (typeof(newDataObj) === typeof String ? newDataObj : null);
  ...
  const hasAnswers = schema && schema.answers && schema.answers.length;
  ...
  const manualCheck = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setPreferredValue(e.target.checked ? schema.answers[0] : null);
    if (hasContention) {
      setResolved(true);
    }
  }, [hasContention, schema.answers]);
  ...
  let inputColor;
  const isCheckbox = schema && schema.displayControl?.answerLayout?.type === 'CHECK_BOX';
  ...
  const isSelect = schema && schema.answers && schema.answers.length > 1 && schema.answers[0] !== null;
        ...
        <div style={isCheckbox ? checkboxParentContainerStyles : ({} as any)}>
          <div style={{ fontSize: '0.9rem', display: isCheckbox ? 'inline-flex' : 'block' }}>
            <Tooltip placement="top" label={<div>Click to highlight evidences</div>}>
              <strong style={{ cursor: 'pointer', userSelect: 'none' }} onClick={() => { onEvidenceClicked?.(path, { "question": name, "answer": preferredValue, "verbatim_source": newDataObj?.verbatim_source }) }}>{schema.question}&nbsp;({schema.localQuestionCode})</strong>
            </Tooltip>
          </div>
          {/* <div>Pref: {preferredValue}</div> */}
          {!isCheckbox && (isSelect ? (
            ...
            <ChakraSelect
              ...
              onKeyDown={onKeyDown}
            >
              {/*[null].concat(schema.answers).map((answer: string | null) => <option value={answer !== null ? answer : ''}>{answer === null ? 'Not selected' : answer}</option>)*/}
              {[{ text: 'Not selected', code: null }].concat(schema.answers).map((answer: any) => <option key={answer.text !== null ? answer.text : 'none'} value={answer.text !== null ? answer.text : ''}>{answer.text === null ? 'Not selected' : answer.text}</option>)}
            </ChakraSelect>
          ) : (
            <Input
              ref={inputRef}
          ...
          {isCheckbox && (
            ...
            >
              <Checkbox
                ref={inputRef}
                isChecked={preferredValue ? Boolean(preferredValue) : false}
