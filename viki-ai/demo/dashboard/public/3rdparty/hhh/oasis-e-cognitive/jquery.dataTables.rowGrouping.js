    ...
    $.fn.rowGrouping = function (options) {
        ...
        return this.each(function (index, elem) {
            ...
            function fnCreateGroupRow(sGroupCleaned, sGroup, iColspan) {
				...
				nGroup.setAttribute("class", "group-item");
			...
			function _fnDrawCallBackWithGrouping (oSettings) {
                ...
                var nTrs = this.$('tr', { 'filter' : 'applied', 'page' : 'current' });
...
//                          sGroupData = oSettings.aoData[oSettings.aiDisplay[iDisplayIndex]]._aData[oSettings.aoColumns[properties.iGroupingColumnIndex].mDataProp];
                        ...
                        if(properties.bGroupPlainText)
                            sGroupData = $("<div>" + sGroupData + "</div>").text(); // ignore html content for sorting purposes (wrap content in a div in case it is plain text to avoid syntax errors)

                        //Issue 31 - End fix provided by Fabien Taysse 
						
						if(fnGetGroup === undefined)
            ...
            if( properties.iGroupingOrderByColumnIndex != -1 ) {
                oTable.fnSettings().aaSorting = [];
			    oTable.fnSettings().aaSortingFixed = [];
            }
			// remove all callbacks, because we don't want any old
			// grouping data to be kept
			var drawCallback = oTable.fnSettings().aoDrawCallback;
			var i = drawCallback.length;
			
			while(i--)
			{
				if(drawCallback[i].sName === "fnRowGrouping")
					drawCallback.splice(i, 1);
			}
			
			if(properties.iGroupingColumnIndex > -1)
			{
	...
	*/
            ...
            // Fix for page and sorting state persistance using rowGrouping
			if( oTable.fnSettings().oInit['bStateSave'] && oTable['fnStandingRedraw'])
