...
CREATE TABLE IF NOT EXISTS `viki-env-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped`
(
...
MERGE INTO `viki-env-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-env-app-wsky`.`firestore_sync`.`pg_medispan_meds_raw_changelog`
