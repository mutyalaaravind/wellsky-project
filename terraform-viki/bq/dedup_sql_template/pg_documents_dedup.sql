...
CREATE TABLE IF NOT EXISTS `viki-env-app-wsky`.`firestore_sync`.`pg_documents_deduped`
(
...
MERGE INTO `viki-env-app-wsky`.`firestore_sync`.`pg_documents_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-env-app-wsky`.`firestore_sync`.`pg_documents_raw_changelog`
