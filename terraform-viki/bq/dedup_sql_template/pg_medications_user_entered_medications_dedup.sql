...
CREATE TABLE IF NOT EXISTS `viki-env-app-wsky`.`firestore_sync`.`pg_medications_user_entered_medications_deduped`
(
...
MERGE INTO `viki-env-app-wsky`.`firestore_sync`.`pg_medications_user_entered_medications_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-env-app-wsky`.`firestore_sync`.`pg_medications_user_entered_medications_raw_changelog`
