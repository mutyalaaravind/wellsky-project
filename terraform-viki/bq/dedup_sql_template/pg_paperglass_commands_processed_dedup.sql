...
CREATE TABLE IF NOT EXISTS `viki-env-app-wsky`.`firestore_sync`.`pg_paperglass_commands_processed_deduped`
(
...
MERGE INTO `viki-env-app-wsky`.`firestore_sync`.`pg_paperglass_commands_processed_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-env-app-wsky`.`firestore_sync`.`pg_paperglass_commands_processed_raw_changelog`
