...
CREATE TABLE IF NOT EXISTS `viki-env-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped`
(
...
MERGE INTO `viki-env-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-env-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_raw_changelog`
