...
CREATE TABLE IF NOT EXISTS `viki-env-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped`
(
...
MERGE INTO `viki-env-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-env-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_raw_changelog`
