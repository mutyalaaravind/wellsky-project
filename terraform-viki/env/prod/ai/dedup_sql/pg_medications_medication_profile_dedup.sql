...
CREATE TABLE IF NOT EXISTS `viki-prod-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped`
(
...
MERGE INTO `viki-prod-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-prod-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_raw_changelog`
