...
CREATE TABLE IF NOT EXISTS `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_deduped`
(
...
MERGE INTO `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_raw_changelog`
