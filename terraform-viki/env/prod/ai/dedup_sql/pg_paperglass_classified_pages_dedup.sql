...
CREATE TABLE IF NOT EXISTS `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_classified_pages_deduped`
(
...
MERGE INTO `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_classified_pages_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_classified_pages_raw_changelog`
