...
CREATE TABLE IF NOT EXISTS `viki-prod-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped`
(
...
MERGE INTO `viki-prod-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-prod-app-wsky`.`firestore_sync`.`pg_medispan_meds_raw_changelog`
