...
CREATE TABLE IF NOT EXISTS `viki-prod-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped`
(
...
MERGE INTO `viki-prod-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-prod-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_raw_changelog`
