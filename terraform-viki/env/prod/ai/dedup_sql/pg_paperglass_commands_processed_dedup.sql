...
CREATE TABLE IF NOT EXISTS `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_commands_processed_deduped`
(
...
MERGE INTO `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_commands_processed_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-prod-app-wsky`.`firestore_sync`.`pg_paperglass_commands_processed_raw_changelog`
