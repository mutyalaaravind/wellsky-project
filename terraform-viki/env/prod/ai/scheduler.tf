...
resource "google_cloud_scheduler_job" "paperglass_e2e_medicationextraction_test_job" {
  ...
  schedule    = "0 0 * * 0"  # Every at midnight on Sunday (turned off during the week to not interfere with load test statistics)
  ...
  http_target {
    http_method = "POST"
    uri         = "${module.paperglass_api.url}/api/orchestrate/medication_extraction/test"
    headers = {
