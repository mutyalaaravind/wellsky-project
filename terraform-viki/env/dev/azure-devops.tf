locals {
  ...
  provision_app   = true # Set to false to skip provisioning in app_project
  app_wif_prefix  = "${local.name_prefix}-app"
  app_pool_id     = "${local.app_wif_prefix}-ado-mediwareis"
  app_sa_email    = "${local.app_pool_id}@${var.app_project_id}.iam.gserviceaccount.com"
  app_sa_id       = "projects/${var.app_project_id}/serviceAccounts/${local.app_sa_email}"
  app_ado_subject = "sc://mediwareis/VIKI/${var.app_project_id}"
}
...
module "ado_wif_app_project" {
  count  = local.provision_app ? 1 : 0
  source = "git@github.com:mediwareinc/terraform-epe-modules.git//ado-workload-identity-federation?ref=1.1.2"

  project_id = var.app_project_id

  name_prefix = local.app_wif_prefix

  ado = {
    service_connection_name = var.app_project_id
  }

  labels = var.labels
}

resource "google_service_account_iam_member" "ado_app_identity_user" {
  count = local.provision_app ? 1 : 0

  service_account_id = local.app_sa_id
  role               = "roles/iam.workloadIdentityUser"
  member             = "principal://iam.googleapis.com/projects/${data.google_project.app-project.number}/locations/global/workloadIdentityPools/${local.app_pool_id}/subject/${local.app_ado_subject}"
...
resource "google_project_iam_member" "ado_app_run_developer" {
  count   = local.provision_app ? 1 : 0
  project = var.app_project_id
...
resource "google_project_iam_member" "ado_app_sa_user" {
  count   = local.provision_app ? 1 : 0
  project = var.app_project_id
...
resource "google_artifact_registry_repository_iam_member" "app_artifact_registry_writer" {
  count      = local.provision_app ? 1 : 0
  repository = google_artifact_registry_repository.images[0].name
  location   = google_artifact_registry_repository.images[0].location
  project    = var.mgmt_project_id
...
resource "google_artifact_registry_repository_iam_member" "app_artifact_registry_reader" {
  count      = local.provision_app ? 1 : 0
  repository = google_artifact_registry_repository.images[0].name
  location   = google_artifact_registry_repository.images[0].location
  project    = var.mgmt_project_id
