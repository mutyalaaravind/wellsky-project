...
resource "google_project_service" "services" {
  for_each = toset([
    "sqladmin.googleapis.com", # required for cloud sql in cac project
    "appengine.googleapis.com" # required for app engine in cac project
  ])
  ...
  disable_on_destroy         = false # Don't disable the service on destroy since it may still be required
}
...
# enable spanner api in management project needed for CAC
resource "google_project_service" "spanner_api" {
  for_each = toset([var.mgmt_project_id])
  project  = each.value
