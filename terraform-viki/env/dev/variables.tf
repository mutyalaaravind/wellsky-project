...
variable "okta_agent_hub_users" {
  description = "Users who will be assigned as members of Agent Hub group and will be able to login to Agent Hub with Okta"
  type        = set(string)
}
...
variable "skysense_scribe_support_tool_app_version" {
  type        = string
  default     = ""
...
variable "summarization_redirect_uris" {
  type        = list(string)
  default     = []
  description = "List of redirect_uri values for Summarization"
}

variable "summarization_trusted_origins" {
  description = "List of trusted origin URLs for Summarization"
  type        = list(string)
}
...
variable "collab_admins" {
  ...
  default     = ["admin@example.com"] # Provide a dummy admin email as default
}
...
variable "rcmedge_config" {
  type = object({
    app_project_id      = string
    dmz_project_id      = string
    mgmt_project_id     = string
    region              = string
    env                 = string
    rcmedge_version     = string
    allow_public_access = bool
    labels              = map(string)
    managers            = list(string)
  })
  description = "Configuration for rcmedge module"
}

variable "repo_scanner_config" {
  type = object({
    app_project_id  = string
    mgmt_project_id = string
    region          = string
  ...
  description = "Configuration for code scanner module"
}
