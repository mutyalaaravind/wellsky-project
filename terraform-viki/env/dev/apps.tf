module "ai" {
  ...
  okta_admin_group_id        = module.okta_admin_group.group.id
  okta_regular_users_group_id = module.okta_regular_users_group.group.id
  okta_app_id                = module.okta_admin_spa.okta_spa_app.client_id
  ...
  skysense_scribe_support_tool_app_version      = var.skysense_scribe_support_tool_app_version
  ...
  demo_api_url              = "https://ai-demo-api-${local.cloud_run_base_url}"
  autoscribe_api_url        = "https://ai-autoscribe-api-${local.cloud_run_base_url}"
  autoscribe_widget_url     = "https://ai-autoscribe-widget-${local.cloud_run_base_url}"
  extract_and_fill_widget_url = "https://ai-extract-and-fill-widget-${local.cloud_run_base_url}"
  paperglass_widget_url     = "https://ai-paperglass-widget-${local.cloud_run_base_url}"
  paperglass_api_secondary_url = "https://ai-paperglass-api-secondary-${local.cloud_run_base_url}"
  ...
  alloydb_ip_address             = var.alloydb_ip_address
...
module "rcmedge" {
  source              = "./rcmedge"
  app_project_id      = var.rcmedge_config.app_project_id
  dmz_project_id      = var.rcmedge_config.dmz_project_id
  mgmt_project_id     = var.rcmedge_config.mgmt_project_id
  region              = var.rcmedge_config.region
  env                 = var.rcmedge_config.env
  rcmedge_version     = var.rcmedge_config.rcmedge_version
  allow_public_access = var.rcmedge_config.allow_public_access
  labels              = var.rcmedge_config.labels
  managers            = var.rcmedge_config.managers
}
...
module "repo_scanner" {
  ...
  app_project_id  = var.repo_scanner_config.app_project_id
  mgmt_project_id = var.repo_scanner_config.mgmt_project_id
  region          = var.repo_scanner_config.region
  env             = var.repo_scanner_config.env
  labels          = var.repo_scanner_config.labels
  managers        = var.repo_scanner_config.managers
  image_version   = var.repo_scanner_config.image_version
  network_tags    = var.repo_scanner_config.network_tags
  vpc_connector   = var.repo_scanner_config.vpc_connector
