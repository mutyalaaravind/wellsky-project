...
# Grant summarization agents service account access to existing AI resources
resource "google_project_iam_member" "summarization_agents_ai_access" {
  project = var.app_project_id
  role    = "roles/aiplatform.admin"
  member  = module.ai_sa.iam_email
}

# Grant summarization agents service account access to existing storage resources
resource "google_project_iam_member" "summarization_agents_storage_access" {
  project = var.app_project_id
  role    = "roles/storage.admin"
  member  = module.ai_sa.iam_email
}

# Grant summarization agents service account access to existing Firestore resources
resource "google_project_iam_member" "summarization_agents_firestore_access" {
  project = var.app_project_id
  role    = "roles/datastore.user"
  member  = module.ai_sa.iam_email
}

# Grant summarization agents service account access to existing BigQuery resources
resource "google_project_iam_member" "summarization_agents_bigquery_access" {
  project = var.app_project_id
  role    = "roles/bigquery.dataEditor"
  member  = module.ai_sa.iam_email
}

resource "google_project_iam_member" "summarization_agents_bigquery_jobs" {
  project = var.app_project_id
  role    = "roles/bigquery.jobUser"
  member  = module.ai_sa.iam_email
}

# Grant summarization agents service account access to existing Cloud Trace resources
resource "google_project_iam_member" "summarization_agents_trace_access" {
  project = var.app_project_id
  role    = "roles/cloudtrace.agent"
  member  = module.ai_sa.iam_email
}

# Grant summarization agents dev service account access to AI Platform
resource "google_project_iam_member" "summarization_agents_dev_ai_access" {
  project = var.app_project_id
  role    = "roles/aiplatform.admin"
  member  = "serviceAccount:summarization-agents-dev-sa@viki-dev-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents dev service account Vertex AI user permissions
resource "google_project_iam_member" "summarization_agents_dev_vertex_ai_user" {
  project = var.app_project_id
  role    = "roles/aiplatform.user"
  member  = "serviceAccount:summarization-agents-dev-sa@viki-dev-app-wsky.iam.gserviceaccount.com"
}

# Grant sonny summarization agents dev service account Vertex AI user permissions
resource "google_project_iam_member" "sonny_summarization_agents_dev_vertex_ai_user" {
  project = var.app_project_id
  role    = "roles/aiplatform.user"
  member  = "serviceAccount:summarization-agents-dev-sa@sonny-dev-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents dev service account ability to invoke Cloud Run services
resource "google_project_iam_member" "summarization_agents_dev_cloud_run_invoker" {
  project = var.app_project_id
  role    = "roles/run.invoker"
  member  = "serviceAccount:summarization-agents-dev-sa@viki-dev-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents dev service account ability to read and write to Cloud Storage buckets
resource "google_project_iam_member" "summarization_agents_dev_storage_object_admin" {
  project = var.app_project_id
  role    = "roles/storage.objectAdmin"
  member  = "serviceAccount:summarization-agents-dev-sa@viki-dev-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents dev service account access to Firestore DB
resource "google_project_iam_member" "summarization_agents_dev_firestore_access" {
  project = var.app_project_id
  role    = "roles/datastore.user"
  member  = "serviceAccount:summarization-agents-dev-sa@viki-dev-app-wsky.iam.gserviceaccount.com"
}
