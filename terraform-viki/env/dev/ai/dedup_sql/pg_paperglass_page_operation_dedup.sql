...
CREATE TABLE IF NOT EXISTS `viki-dev-app-wsky`.`firestore_sync`.`pg_paperglass_page_operation_deduped`
(
...
MERGE INTO `viki-dev-app-wsky`.`firestore_sync`.`pg_paperglass_page_operation_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-dev-app-wsky`.`firestore_sync`.`pg_paperglass_page_operation_raw_changelog`
