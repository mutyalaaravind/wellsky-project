...
CREATE TABLE IF NOT EXISTS `viki-dev-app-wsky`.`firestore_sync`.`pg_medications_imported_medications_deduped`
(
...
MERGE INTO `viki-dev-app-wsky`.`firestore_sync`.`pg_medications_imported_medications_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-dev-app-wsky`.`firestore_sync`.`pg_medications_imported_medications_raw_changelog`
