...
CREATE TABLE IF NOT EXISTS `viki-dev-app-wsky`.`firestore_sync`.`pg_paperglass_commands_deduped`
(
...
MERGE INTO `viki-dev-app-wsky`.`firestore_sync`.`pg_paperglass_commands_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-dev-app-wsky`.`firestore_sync`.`pg_paperglass_commands_raw_changelog`
