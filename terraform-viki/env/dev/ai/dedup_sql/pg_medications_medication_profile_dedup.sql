...
CREATE TABLE IF NOT EXISTS `viki-dev-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped`
(
...
MERGE INTO `viki-dev-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-dev-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_raw_changelog`
