...
module "skysense_scribe_support_tool" {
  ...
  volumes = {
    envjson = {
      secret = {
        secret_name = module.skysense_scribe_support_tool_envjson_config.name
        items = [{
          key  = "latest"
          path = "env.json"
        }]
  ...
  depends_on          = [module.skysense_scribe_support_tool_envjson_config]
}
