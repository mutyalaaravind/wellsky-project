module "admin_api" {
  ...
  env = {
    ...
    OKTA_ADMIN_GROUP_ID                = var.okta_admin_group_id
    OKTA_REGULAR_USERS_GROUP_ID        = var.okta_regular_users_group_id
    OKTA_DISABLE                       = var.okta_disable
    OKTA_DOMAIN                        = regex("https://([^/]+)", var.okta_service_issuer_url)[0]

    OKTA_AUTO_ASSIGN_ENABLED           = "true"
    OKTA_USER_VALIDATION_ENABLED       = "true"
    OKTA_FAIL_ON_USER_NOT_FOUND        = "true"
  }
  secrets = toset([
    {
    ...
    {
      key     = "OKTA_API_TOKEN",
      name    = module.secret["okta_api_token"].name,
      version = "latest",
    }
  ])
...
module "admin_ui" {
  ...
  volumes = {
    envjson = {
      secret = {
        secret_name = module.admin_ui_envjson_config.name
        items = [{
          key  = "latest"
          path = "env.json"
        }]
  ...
  concurrency         = var.cloud_run_services.admin_ui.concurrency
  timeout             = var.cloud_run_services.admin_ui.timeout
  depends_on          = [module.admin_ui_envjson_config]
}
