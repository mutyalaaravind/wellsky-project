...
resource "google_eventarc_trigger" "firestore_trigger_scribe_default" {
  ...
  matching_criteria {
    attribute = "document"
    value     = "skysense-scribe-upload-chunks/{docId}/chunks/{chunkId}"
    operator  = "match-path-pattern"
  }
...
# Commented out due to apply error:  Error 403: User not authorized to perform this action
# resource "google_eventarc_trigger" "external_files_hhh_pubsub_trigger" {
#   project  = var.app_project_id
#   name     = format("%s-%s", var.env, "hhh-external-files")
#   location = var.region
#   matching_criteria {
#     attribute = "type"
#     value     = "google.cloud.pubsub.topic.v1.messagePublished"
...
#   transport {
#     pubsub {
#       topic = "projects/hhh-${var.env}-app-wsky/topics/HHH-AM-Attachments"
#     }
#   }
...
# resource "google_eventarc_trigger" "orchestration_extraction_trigger" {
#   project  = var.app_project_id
#   name     = format("%s-%s", var.env, "orchestration-extraction-trigger")
#   location = var.region
#   matching_criteria {
#     attribute = "type"
#     value = "google.cloud.pubsub.topic.v1.messagePublished"
