...
module "autoscribe_widget" {
  ...
  volumes = {
    envjson = {
      secret = {
        secret_name = module.autoscribe_widget_envjson_config.name
        items = [{
          key  = "latest"
          path = "env.json"
        }]
  ...
  labels              = var.labels
  ingress             = "all"
  allow_public_access = var.cloud_run_services.autoscribe_widget.allow_public_access
  ...
  concurrency         = var.cloud_run_services.autoscribe_widget.concurrency
  timeout             = var.cloud_run_services.autoscribe_widget.timeout  
  depends_on          = [module.autoscribe_widget_envjson_config]
}
