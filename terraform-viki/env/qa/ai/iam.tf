...
resource "google_project_iam_member" "summarization_agents_qa_ai_access" {
  project = var.app_project_id
  role    = "roles/aiplatform.admin"
  member  = "serviceAccount:summarization-agents-qa-sa@viki-qa-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents qa service account Vertex AI user permissions
resource "google_project_iam_member" "summarization_agents_qa_vertex_ai_user" {
  project = var.app_project_id
  role    = "roles/aiplatform.user"
  member  = "serviceAccount:summarization-agents-qa-sa@viki-qa-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents qa service account ability to invoke Cloud Run services
resource "google_project_iam_member" "summarization_agents_qa_cloud_run_invoker" {
  project = var.app_project_id
  role    = "roles/run.invoker"
  member  = "serviceAccount:summarization-agents-qa-sa@viki-qa-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents qa service account ability to read and write to Cloud Storage buckets
resource "google_project_iam_member" "summarization_agents_qa_storage_object_admin" {
  project = var.app_project_id
  role    = "roles/storage.objectAdmin"
  member  = "serviceAccount:summarization-agents-qa-sa@viki-qa-app-wsky.iam.gserviceaccount.com"
}

# Grant summarization agents qa service account access to Firestore DB
resource "google_project_iam_member" "summarization_agents_qa_firestore_access" {
  project = var.app_project_id
  role    = "roles/datastore.user"
