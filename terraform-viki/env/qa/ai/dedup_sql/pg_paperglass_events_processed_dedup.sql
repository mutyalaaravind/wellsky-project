...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_events_processed_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_events_processed_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_events_processed_raw_changelog`
