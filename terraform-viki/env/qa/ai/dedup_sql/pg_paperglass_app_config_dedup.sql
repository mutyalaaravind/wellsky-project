...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_app_config_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_app_config_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_app_config_raw_changelog`
