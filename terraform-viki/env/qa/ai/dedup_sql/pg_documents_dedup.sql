...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_documents_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_documents_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_documents_raw_changelog`
