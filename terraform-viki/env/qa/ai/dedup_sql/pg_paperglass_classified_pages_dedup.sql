...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_classified_pages_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_classified_pages_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_classified_pages_raw_changelog`
