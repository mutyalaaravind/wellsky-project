...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_medispan_meds_raw_changelog`
