...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_medications_medication_profile_raw_changelog`
