...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_raw_changelog`
