...
CREATE TABLE IF NOT EXISTS `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_deduped`
(
...
MERGE INTO `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-qa-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_raw_changelog`
