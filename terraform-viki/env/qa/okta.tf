...
module "okta_hhh_app" {
  ...
  app_client_id = "${local.namespace}.paperglass.${var.env}" # DO NOT CHANGE - This is in use by HHH.  Changeing will force the creation of a new client ID/Secret which will need to be coordinated with HHH
  app_status    = "ACTIVE"
}
...
# New Okta app for summarization agents
module "okta_summarization_agents_app" {
  source = "git@github.com:mediwareinc/terraform-okta//okta-service-app?ref=v1.9.4"

  app_name      = "Summarization Agents Client(${title(var.env)})"
  app_client_id = "${local.namespace}.${var.env}.summarization-agents"
  app_status    = "ACTIVE"
}

# Summarization Agents User Group
module "okta_summarization_agents_group" {
  source = "git@github.com:mediwareinc/terraform-okta//okta-group?ref=v1.9.0"

  group_name = "Summarization Agents Users (VIKI ${title(var.env)})"
  group_desc = "Users who can access Summarization Agents services"
}

# Policy for summarization agents
module "okta_auth_server_policy_summarization_agents" {
  source = "git@github.com:mediwareinc/terraform-okta.git//okta-auth-server-policy?ref=v1.8.8"

  okta_auth_server_name = module.okta_auth_server.okta_auth_server.name
  policy_allowed_clients = [
    module.okta_summarization_agents_app.okta_service_app.client_id,
  ]
  policy_desc = "Policy for Summarization Agents clients"
  policy_name = "Policy for Summarization Agents clients (${var.env})"

  depends_on = [module.okta_auth_server]
}

# Policy rule for summarization agents
module "okta_auth_server_policy_rule_summarization_agents" {
  source = "git@github.com:mediwareinc/terraform-okta.git//okta-auth-server-policy-rule?ref=v1.8.8"

  grant_type_whitelist           = ["client_credentials"]
  okta_auth_server_name          = module.okta_auth_server.okta_auth_server.name
  policy_id                      = module.okta_auth_server_policy_summarization_agents.auth_server_policy.id
  access_token_lifetime_minutes  = 60
  refresh_token_lifetime_minutes = 3600
  refresh_token_window_minutes   = 3600
  rule_name                      = "Summarization Agents policy to call APIs"
  scope_whitelist                = [local.okta_scopes.summarization-agents]

  depends_on = [module.okta_auth_server]
}

# Group assignment for summarization agents app
module "okta_summarization_agents_group_assignment" {
  source = "git@github.com:mediwareinc/terraform-okta//okta-assign-app-to-group?ref=v1.9.0"

  app_id   = module.okta_summarization_agents_app.okta_service_app.id
  group_id = module.okta_summarization_agents_group.group.id
}

# User assignments to summarization agents group
data "external" "okta_summarization_agents_user" {
  for_each = var.summarization_agents_users
  program  = ["sh", "${path.module}/find_okta_user.sh"]
  query = {
    okta_api_token = var.okta_api_token
    email          = each.value
...
resource "okta_group_memberships" "summarization_agents_users_members" {
  group_id = module.okta_summarization_agents_group.group.id
  users    = [for user in data.external.okta_summarization_agents_user : user.result.user_id]
}
