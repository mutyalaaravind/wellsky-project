...
module "github_builder" {
  count   = local.provision_mgmt ? 1 : 0 # TODO Fix this as it is overwritten on promote env
...
resource "google_service_account_iam_member" "wif_sa_member" {
  for_each           = local.provision_mgmt ? toset(slice(local.repos, 1, length(local.repos))) : toset([]) # TODO Fix this as it is overwritten on promote env
