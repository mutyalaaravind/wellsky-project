...
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_documents_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_documents_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_documents_raw_changelog`
