...
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_medications_imported_medications_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_medications_imported_medications_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_medications_imported_medications_raw_changelog`
