...
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_page_operation_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_page_operation_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_page_operation_raw_changelog`
