...
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_medispan_meds_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_medispan_meds_raw_changelog`
