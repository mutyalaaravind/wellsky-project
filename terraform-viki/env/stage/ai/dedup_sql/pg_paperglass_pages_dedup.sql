--- query firestore_sync pg_paperglass_pages_raw table for last 1 hour using timestamp field and do a merge/upsert into a table "pg_paperglass_pages_deduped"
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_pages_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_pages_deduped` AS target 
...
USING (
  SELECT * FROM (
    SELECT 
      ...
      CAST(JSON_EXTRACT(data, '$.number') AS INT64) as data_number,
      JSON_EXTRACT_SCALAR(data, '$.ocr_storage_uris.raw') as data_ocr_storage_uris_raw,
      JSON_EXTRACT_SCALAR(data, '$.patient_id') as data_patient_id,
      JSON_EXTRACT_SCALAR(data, '$.rotation') as data_rotation,
      JSON_EXTRACT_SCALAR(data, '$.storage_uri') as data_storage_uri,
      JSON_EXTRACT_SCALAR(data, '$.tenant_id') as data_tenant_id,
      JSON_EXTRACT_SCALAR(data, '$.text') as data_text,
      document_id,
      ROW_NUMBER() OVER (PARTITION BY document_id ORDER BY timestamp DESC) as rn
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_pages_raw_changelog`
...
WHEN MATCHED AND TIMESTAMP(target.modified_at) < TIMESTAMP(source.data_modified_at) THEN 
  UPDATE SET
      ...
      target.number = source.data_number,
      target.ocr_storage_uris_raw = source.data_ocr_storage_uris_raw,
      target.patient_id = source.data_patient_id,
      target.rotation = source.data_rotation,
      target.storage_uri = source.data_storage_uri,
      target.tenant_id = source.data_tenant_id,
      target.text = source.data_text
...
WHEN NOT MATCHED THEN 
  ...
  VALUES (
    ...
    source.data_number,
    source.data_ocr_storage_uris_raw,
    source.data_patient_id,
    source.data_rotation,
