...
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_log_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_log_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_instance_log_raw_changelog`
