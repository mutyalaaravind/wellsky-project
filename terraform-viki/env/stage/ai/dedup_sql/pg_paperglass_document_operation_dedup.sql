--- query firestore_sync pg_paperglass_document_operation_raw table for last 1 hour using timestamp field and do a merge/upsert into a table "pg_paperglass_document_operation_deduped"
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_document_operation_raw_changelog`
