...
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_medications_host_attachments_raw_changelog`
