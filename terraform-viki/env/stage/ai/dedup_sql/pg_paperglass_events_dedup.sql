--- query firestore_sync pg_paperglass_events_raw table for last 1 hour using timestamp field and do a merge/upsert into a table "pg_paperglass_events_deduped"
CREATE TABLE IF NOT EXISTS `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_events_deduped`
(
...
MERGE INTO `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_events_deduped` AS target 
...
USING (
  SELECT * FROM (
    ...
    FROM `viki-stage-app-wsky`.`firestore_sync`.`pg_paperglass_events_raw_changelog`
