...
module "paperglass_external_events" {
  ...
  command = [
    ...
    "--host", "0.0.0.0",
    "--port", "8080",
    "--workers", tostring(var.cloud_run_services.paperglass_external_api.uvicorn_workers),
  ]
  env = merge(local.common_env, {
    # Service-specific overrides
  ...
  labels              = var.labels
  ingress             = "all"
  allow_public_access = var.cloud_run_services.paperglass_external_api.allow_public_access
  organization_id     = var.organization_id
  min_instances       = var.cloud_run_services.paperglass_external_api.min_instances
  max_instances       = var.cloud_run_services.paperglass_external_api.max_instances == "unlimited" ? null : var.cloud_run_services.paperglass_external_api.max_instances
  concurrency         = var.cloud_run_services.paperglass_external_api.concurrency
  memory              = var.cloud_run_services.paperglass_external_api.memory
  cpus                = var.cloud_run_services.paperglass_external_api.cpus
  project_id          = var.app_project_id
  timeout             = var.cloud_run_services.paperglass_external_api.timeout
}

module "paperglass_widget_envjson" {
  source              = "git@github.com:mediwareinc/terraform-gcp-secret.git?ref=v0.4.1"
  labels              = var.labels
  managers            = []
  accessors           = ["serviceAccount:${local.ai_sa_email}"]
  name                = "ai-paperglass-widget-${var.env}-envjson"
  project_id          = var.app_project_id
...
module "paperglass_widget" {
  ...
  volumes = {
    envjson = {
      secret = {
        secret_name = module.paperglass_widget_envjson.name
        items = [{
          key  = "latest"
          path = "env.json"
        }]
  ...
  labels              = var.labels
  ingress             = "all"
  allow_public_access = var.cloud_run_services.paperglass_widget.allow_public_access
  organization_id     = var.organization_id
  ...
  project_id          = var.app_project_id
  depends_on          = [module.paperglass_widget_envjson]
}